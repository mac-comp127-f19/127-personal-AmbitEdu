...
public class Library {
    ...
    public List<Book> findShortestBooks(int n) {
        ...
        for(Book b : books) {
            if(newBooks.isEmpty())
            {
                ...
                for(int i = 0; i < newBooks.size(); i++) {
                    if (b.getPageCount() < newBooks.get(i).getPageCount()) {
                        newBooks.add(i, b);
                        i = newBooks.size(); //basically a break
                    }
                }
            }
        }
        ...
        int count;
        if(newBooks.size() < n) { count = newBooks.size(); } else { count = n; }

        for(int i = 0; i < count; i++) {
            result.add(newBooks.get(i));
        }
        return result;
    }
    ...
    public Book findBookWithShortestTitle() {
        ...
        return books.stream()
            ...
            .orElse(null);
         */
        if(books.isEmpty()) { return null; } else {
            Book shortestTitle = books.get(0);
            for(Book b : books) {
                if(b.getTitle().length() < shortestTitle.getTitle().length()) {
                    shortestTitle = b;
                }
            }
    ...
    public boolean hasAuthorWrittenInGenre(String author, String genre) {
        ...
        return books.stream()
                .anyMatch(b -> b.getAuthor().equals(author) && b.getGenres().contains(genre));
    }
