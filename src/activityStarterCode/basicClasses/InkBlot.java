...
public class InkBlot {
     ...
     * draw on.
     */
    public InkBlot(int canvasSize) {
        canvas = new CanvasWindow("InkBlot", canvasSize, canvasSize);
    }
    ...
    public void generateInkBlot() {
        int colorInit = rand.nextInt(256);
        RandomWalk red = new RandomWalk(colorInit, 256);
        RandomWalk green = new RandomWalk(colorInit, 256);
        RandomWalk blue = new RandomWalk(colorInit, 256);

        RandomWalk x = new RandomWalk(150, 300);
        RandomWalk y = new RandomWalk(150, 300);

        Line currentLine;

        for(int i = 0; i < 15000; i++) {
            currentLine = advance(x, y);
            currentLine.setStrokeColor(getColor(red, green, blue));
            canvas.add(currentLine);
            canvas.add(mirror(currentLine));
        }
    }
